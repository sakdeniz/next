...
data: function () {
		...
		var i=0;
			...
			axios.post(window.hostname+'listproposals',{token:window.token,rpcport:window.rpcport},window.config).then(function(res2)
			{
				var aLen=0;
				axios.post("http://navcommunity.net/api/getproposals.php",{},window.config).then(function(res0)
				{
					jsonQ.each(res0.data, function (key, value)
					{
						...
						{
							array_proposals_remote.push({hash:value2.hash,image:value2.image,featured:value2.featured});
							aLen++;
						});
					});
							...
							if (key2=="status")
							{
								nav_address_list.filter(function (v, k)
								{
									if (v["address"]==paymentAddress)
									{
										...
										array_proposals.push({hash:hash,desc:desc,amount:amount,paymentAddress:paymentAddress,deadline:deadline,votesYes:votesYes,votesNo:votesNo,status:status,image:image,textfeatured:textfeatured,classfeatured:classfeatured});
									}
								});
							}
						});
					});
					if (array_proposals.length<1) proposal_info="You don't have any proposal."; else proposal_info="You have " + array_proposals.length + " proposal";
				}).catch(function(err)
				{
					console.log(err)
				})
