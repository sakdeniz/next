      ...
      <div class="row">
        <div class="ui link cards">
			<div class="card" style="margin:10px;height:400px !important;" v-for="item in getStoreItems(items)">
				...
				<div class="extra content">
					<span class="meta right floated""><a class="ui purple tag label">{{item.store_item_price}} NAV</a></span>     
  ...
  methods: {
	...
	buy:function(vitem)
	{
		...
		const {value: formValues} = swal({
			...
			html:
				'<form class="ui form"><h5>Buy</h5><h5>'+vitem.store_item_name+' ('+vitem.store_item_price+' NAV)</h5><code>'+vitem.store_item_payment_address+'</code><h4 class="ui dividing header">Shipping Information</h4>'+
				...
				preConfirm: () => {return [document.getElementById('swal-input1').value,
					...
					}).then(formValues =>
					{
						...
						var order={store_item_id:vitem.store_item_id,
								   ...
								   email:email,
								   notes:notes};
						swal({onOpen: () => {swal.showLoading()},allowOutsideClick:false,text: 'Please wait...'});
						this.BuyStoreItems(order).then(function(res)
						{
								...
								if (JSON.stringify(vm.buystoreitems)!="[]")
								{
									...
									{
										swal({type:"success",title:"Payment Success!",html:"<div style='text-align:left'>"+
										"Item ID:"+item.id+
										"<br>Price:"+item.price+
									...
									{
										swal({type:"error",title:"Error",html:"<div style='text-align:left'>" + item.message + "</div>"});
									}
								}
								else
								{
									swal({type:"error",title:"Error",html:"<div style='text-align:left'>An error occured while connecting to server...</div>"});
								}					
					})
			        .catch(function(err)
					{
						swal({type:"error",title:"Error",html:"<div style='text-align:left'>" + err["message"] + "</div>"});
					});
				})
	},
    getStoreItems: (items) =>
